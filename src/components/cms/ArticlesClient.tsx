...
import { Plus, Search, Edit2, Trash2, X, FileText, Save, Loader2, Upload, AlertCircle, Check, Sparkles, ChevronLeft, ChevronRight, Tags } from "lucide-react";
import { z } from "zod";
...
export default function ArticlesClient({ articles, categories }: ArticlesClientProps) {
  ...
  const openEditPanel = (item: ArticleType) => {
    ...
    setPublishedAt(
      item.publishedAt ? new Date(item.publishedAt).toISOString().split("T")[0] : ""
    );
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Could not delete article.";
      alert(msg);
    }
  };
              ...
              {paginatedArticles.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-12 text-center text-foreground/40 italic">
                    No articles found matching the filter parameters.
                  </td>
                </tr>
              ) : (
                paginatedArticles.map((item) => (
                  <tr key={item.id} className="hover:bg-secondary-cream/30 transition-colors">
                    <td className="py-4 px-6">
                      <div className="w-10 h-7 rounded border border-primary-sage/15 bg-secondary-cream overflow-hidden shadow-sm flex items-center justify-center">
                        {item.featuredImage ? (
                          ...
                          <FileText size={12} className="text-primary-sage/40" />
                        )}
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-6 text-foreground/50 max-w-xs truncate">
                      {item.tags ? (
                        <span className="flex items-center gap-1">
                          <Tags size={10} className="text-accent-gold" />
                          {item.tags}
                        </span>
                      ) : (
                        "—"
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-primary-sage/10 px-6 py-4 bg-secondary-cream/35">
            <span className="text-[10px] font-bold text-foreground/50 uppercase tracking-wider">
              Showing Page {currentPage} of {totalPages} ({filteredArticles.length} items total)
            </span>
          ...
          <div className="w-full max-w-4xl bg-[#F8F4EC] h-full shadow-2xl flex flex-col p-6 border-l border-primary-sage/10 animate-slide-in overflow-y-auto">
            ...
            <div className="flex items-center justify-between border-b border-primary-sage/15 pb-4 mb-6 shrink-0">
              <h3 className="font-serif text-lg font-bold text-primary-forest flex items-center gap-2">
                ...
                {editingArticle ? "Modify Wisdom Article" : "New Wisdom Article"}
              </h3>
              <button
                onClick={closePanel}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      placeholder="e.g. 5 Principles of a Mindful Morning"
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      placeholder="e.g. mindful-morning-principles"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      placeholder="e.g. diet, seasonal, daily-routine"
                      className="w-full px-4 py-2.5 text-xs border border-primary-sage/20 rounded-lg bg-[#FCFCFA] focus:outline-none focus:border-accent-gold transition-colors"
                    />
                  </div>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <textarea
                    ...
                    placeholder="Short introduction hook summary displayed in lists..."
                ...
                {/* SEO Sub-section */}
                <div className="p-4 bg-secondary-cream/35 border border-primary-sage/10 rounded-xl space-y-4">
                  <h4 className="font-serif text-sm font-bold text-primary-forest flex items-center gap-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1">
                      <label className="text-[9px] uppercase tracking-widest font-bold text-foreground/50">
                        Meta Title override
                      </label>
                      <input
                        type="text"
                        ...
                        className="w-full px-3 py-2 text-xs border border-primary-sage/20 rounded-lg bg-[#FCFCFA] focus:outline-none focus:border-accent-gold"
                      />
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <input
                        ...
                        placeholder="Google Search brief page description snippet..."
                        className="w-full px-3 py-2 text-xs border border-primary-sage/20 rounded-lg bg-[#FCFCFA] focus:outline-none focus:border-accent-gold"
                      />
                    </div>
                  </div>
