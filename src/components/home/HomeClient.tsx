...
import { ArrowRight, Compass, Sun, Heart, Sparkles, ChevronRight, Star, BookOpen } from "lucide-react";
      ...
      <main className="min-h-screen">
        ...
        <section className="relative h-screen flex items-center justify-center overflow-hidden">
          ...
          <div className="relative z-10 max-w-5xl mx-auto px-6 text-center text-[#FCFCFA] flex flex-col items-center gap-6">
            ...
            >
              Align your modern life with the eternal rhythms of classical Yoga, Ayurvedic wisdom, and high-prana organic meals.
            </motion.p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {featuredYoga.length > 0 ? (
                featuredYoga.map((prog, idx) => {
                  const instructor = prog.instructor;
                  return (
                    <FadeUp key={prog.id} delay={idx * 0.1} className="group flex flex-col bg-[#FCFCFA] rounded-2xl overflow-hidden border border-primary-sage/10 hover:shadow-xl transition-all duration-300 relative">
                      <Link href={`/yoga-classes/${prog.slug}`} className="absolute inset-0 z-10" />
                      <div className="relative h-60 overflow-hidden">
                        ...
                        <span className="absolute top-4 left-4 text-[10px] uppercase tracking-widest font-bold text-[#FCFCFA] bg-primary-forest/85 px-3 py-1 rounded-full backdrop-blur-sm z-20">
                          {prog.category}
                        </span>
                      </div>
                      ...
                      <div className="p-6 flex flex-col gap-3 flex-1">
                        ...
                        <div className="border-t border-primary-sage/10 pt-4 mt-auto flex items-center justify-between z-20">
                          <div className="flex items-center gap-2">
                            {instructor?.profileImage ? (
                              ...
                              <div className="w-8 h-8 rounded-full bg-primary-sage/20 flex items-center justify-center text-[10px] font-bold text-primary-forest">
                          ...
                          <span className="text-[10px] uppercase font-bold tracking-wider text-accent-gold">
                            {prog.isOnline ? "Online" : "In-Studio"}
                          </span>
                        </div>
                      </div>
                    </FadeUp>
                ...
                <div className="col-span-full py-12 flex flex-col items-center justify-center text-center gap-2 text-foreground/50 bg-[#FCFCFA] border border-primary-sage/10 rounded-2xl">
                  <BookOpen size={36} className="stroke-[1.25]" />
                  <p className="text-xs font-medium">No published yoga classes yet</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto flex flex-col gap-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {initialArticles.length > 0 ? (
                  ...
                  <FadeUp key={art.id} delay={idx * 0.1} className="flex flex-col bg-[#F8F4EC] rounded-2xl overflow-hidden border border-primary-sage/10 hover:shadow-lg transition-all duration-300">
                    ...
                    <div className="p-6 flex flex-col gap-3 flex-1">
                      <div className="flex items-center justify-between text-[10px] text-foreground/50 font-medium">
                        ...
                        <span>{calculateReadTime(art.content)} Min Read</span>
                      </div>
                      ...
                      <p className="text-xs text-foreground/75 leading-relaxed font-light line-clamp-3">
                        {art.excerpt || art.content.replace(/<[^>]*>/g, "").slice(0, 130) + "..."}
                      </p>
                ...
                <div className="col-span-full py-12 flex flex-col items-center justify-center text-center gap-2 text-foreground/50">
                  <BookOpen size={36} className="stroke-[1.25]" />
                  <p className="text-xs font-medium">No published articles yet</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {initialBooks.length > 0 ? (
                initialBooks.map((book, idx) => (
                  <FadeUp key={book.id} delay={idx * 0.1} className="bg-[#F8F4EC] rounded-2xl p-6 border border-primary-sage/10 hover:shadow-lg transition-all duration-300 flex items-start gap-4 relative">
                    ...
                    <div className="flex flex-col gap-2 h-full flex-1">
                      ...
                      <Link href={`/online-books/${book.slug}`} className="absolute inset-0 z-10" />
                    </div>
                  </FadeUp>
                ))
              ) : (
                <div className="col-span-full py-12 flex flex-col items-center justify-center text-center gap-2 text-foreground/50">
                  <BookOpen size={36} className="stroke-[1.25]" />
                  <p className="text-xs font-medium">No featured books yet</p>
                </div>
              )}
            </div>
          </div>
        ...
        <section className="py-24 px-6 bg-[#F8F4EC]">
          <div className="max-w-7xl mx-auto flex flex-col gap-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {spotlightInstructors.length > 0 ? (
                spotlightInstructors.map((ins, idx) => (
                  <FadeUp key={ins.id} delay={idx * 0.1} className="bg-[#FCFCFA] rounded-2xl overflow-hidden border border-primary-sage/10 hover:shadow-lg transition-all duration-300 flex flex-col items-center text-center p-6 gap-4 relative">
                    ...
                    <div className="w-28 h-28 rounded-full overflow-hidden border-2 border-accent-gold/45 relative shadow-inner">
                      {ins.profileImage ? (
                        ...
                        <div className="w-full h-full bg-primary-sage/20 flex items-center justify-center text-xl font-bold text-primary-forest">
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-1.5 mt-auto pt-4 border-t border-primary-sage/10 w-full z-20">
                      {ins.certifications ? (
                        ins.certifications.split(",").map(c => c.trim()).slice(0, 2).map((cert) => (
                          <span key={cert} className="text-[9px] text-foreground/60 bg-[#F8F4EC] border border-primary-sage/5 px-2 py-0.5 rounded-sm">
                            {cert}
                          </span>
                        ))
                      ) : (
                        <span className="text-[9px] text-foreground/40 italic">Lineage Guide</span>
                      )}
                    </div>
                  </FadeUp>
                ))
        ...
        <section className="py-20 px-6 bg-primary-forest text-[#FCFCFA] text-center relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-3xl mx-auto flex flex-col items-center gap-6">
            ...
            <p className="text-sm sm:text-base text-[#FCFCFA]/80 leading-relaxed font-light max-w-xl">
              Join 12,000+ souls who receive our weekly essays on Vedic philosophy, seasonal diet tips, and soothing somatic rituals.
            </p>
