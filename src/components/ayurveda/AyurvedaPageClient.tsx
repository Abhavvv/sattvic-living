...
interface AyurvedaPageClientProps {
  initialArticles: Article[];
}

export default function AyurvedaPageClient({ initialArticles }: AyurvedaPageClientProps) {
  ...
  const filteredArticles = initialArticles.filter((art) => {
    ...
    const categoryName = art.category?.name || "Uncategorized";
    const matchesCategory =
      selectedCategoryName === "All" ||
      categoryName.toLowerCase().includes(selectedCategoryName.toLowerCase());
                  ...
                  filteredArticles.map((art, idx) => (
                    ...
                    >
                      <div className="relative h-48 overflow-hidden bg-primary-forest/5">
                        ...
                        {art.category && (
                          <span className="absolute top-4 left-4 text-[9px] uppercase tracking-widest font-bold text-primary-forest bg-[#F8F4EC]/95 px-2.5 py-1 rounded-sm border border-primary-sage/10">
                            {art.category.name}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="p-5 flex flex-col gap-3 flex-1 text-left">
                        <div className="flex items-center justify-between text-[9px] text-foreground/50 font-bold border-b border-primary-sage/5 pb-2">
                          ...
                          <span className="flex items-center gap-1">
                            <Clock size={11} /> {calculateReadTime(art.content)} Min
                          </span>
                        </div>
                        ...
                        <Link href={`/articles/${art.slug}`} className="absolute inset-0 z-10" />
                      </div>
                    </motion.article>
                  ))
                ) : (
